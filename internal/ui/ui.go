...
var (
	...
	pitch          float32 = 55
	dist           float32 = 30
	center                 = mgl32.Vec3{float32(core.Width) * 0.5, 0, float32(core.Height) * 0.5}
...
func Prepare() error {
	...
	if err != nil {
		return fmt.Errorf("build shader program: %w", err)
	}
	Prog = prog
	lineProg, err := newProgram(lineVertSrc, lineFragSrc)
	if err != nil {
		return fmt.Errorf("build line shader program: %w", err)
	}
	LineProg = prog
	...
	GridVAO, GridCount = makeGridLines(width, height)
...
func Render(dt float32, tiles [][]int) {
	win := AppWindow
